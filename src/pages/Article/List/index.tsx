...
const TableList: React.FC<{}> = () => {
  ...
  const columns: ProColumns<TableListItem>[] = [
    {
      title: intl.formatMessage({id: 'app.article.list.table-column-number'}),
      dataIndex: 'index',
      valueType: 'index',
      width: 80,
    ...
    {
      title: intl.formatMessage({id: 'app.article.list.table-column-title'}),
      dataIndex: 'title',
    },
    {
      title: intl.formatMessage({id: 'app.article.list.table-column-category'}),
      dataIndex: 'category',
      hideInSearch: true,
      render: (_, record) => <>{record.category?record.category.name:'-'}</>
    },
    {
      title: intl.formatMessage({id: 'app.article.list.table-column-addtime'}),
      dataIndex: 'addtime',
      hideInSearch: true,
    },
    {
      title: intl.formatMessage({id: 'app.article.list.table-column-tag'}),
      dataIndex: 'tag',
      hideInSearch: true,
      render: (_, record)=> {
    ...
    {
      title: intl.formatMessage({id: 'app.article.list.table-column-oper'}),
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => (
        <>
          <Link to={`/article/edit-${record.id}`}>
            <Button type="link">{intl.formatMessage({id: 'app.article.list.btn-edit'})}</Button>
          </Link>
          ...
          }} >{intl.formatMessage({id: 'app.article.list.btn-del'})}</Button>
        </>
      ),
    },
    /* 以下独立设置的搜索 */
    {
      ...
      renderFormItem: (item, { defaultRender, ...rest }) => {
        return <ArticleCategoryCascader {...rest} />;
      },
    },
    {
      title: intl.formatMessage({id: 'app.article.list.table-column-addtime'}),
    ...
    <PageHeaderWrapper title={false}>
      <IntlProvider value={intlMap[locale]}>
        <ProTable<TableListItem>
            headerTitle= {intl.formatMessage({id: 'app.article.list.table-title'})}
            actionRef={actionRef}
            rowKey="id"
            toolBarRender={() => [
              <Link to="/article/add">
                <Button type="primary">
                  <PlusOutlined /> {intl.formatMessage({id: 'app.article.list.btn-add'})}
                </Button>
              </Link>
            ]}
