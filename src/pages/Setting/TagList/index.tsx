...
const TableList: React.FC<{}> = () => {
    ...
    const onSubmitUpdateModal = async (values: TableListItem, form: FormInstance)  => {
        ...
        const response: ResponseData = await updateList(id, { ...params });
        const { code } = response;
    ...
    const columns: ProColumns<TableListItem>[] = [
      {
        title: intl.formatMessage({id: 'app.settingtaglist.table-column-number'}),
        dataIndex: 'index',
        valueType: 'index',
        width: 80,
      ...
      {
        title: intl.formatMessage({id: 'app.settingtaglist.table-column-name'}),
        dataIndex: 'name',
      },
      {
        title: intl.formatMessage({id: 'app.settingtaglist.table-column-pinyin'}),
        hideInSearch: true,
        render: (_, record) => (
          <>
          {record.pinyin}
      ...
      {
        title: intl.formatMessage({id: 'app.settingtaglist.table-column-oper'}),
        dataIndex: 'option',
        valueType: 'option',
        width: 160,
        render: (_, record) => (
          <>
            <Button type="link" onClick={()=> handleUpdateModalShow(record)} >{intl.formatMessage({id: 'app.settingtaglist.btn-edit'})}</Button>
            ...
            }} >{intl.formatMessage({id: 'app.settingtaglist.btn-del'})}</Button>
          </>
        ),
      },
          ...
          <ProTable<TableListItem> 
            ...
            toolBarRender={() => [
                <Button type="primary"  onClick={() => handleCreateModalVisible(true)}>
                  <PlusOutlined /> {intl.formatMessage({id: 'app.settingtaglist.btn-add'})}
                </Button>
            ]}
