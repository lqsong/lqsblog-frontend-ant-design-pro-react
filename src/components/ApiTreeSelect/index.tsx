...
import { ApiTreeSelectAjaxData, ApiTreeSelectTreeData } from "./data.d";
...
const formatData = (data: ApiTreeSelectAjaxData[], pId: number, pValue: string): DataNode[] => {
    return data.map((item: ApiTreeSelectAjaxData)=> {
        const key = pValue !== '' ? `${ pValue }-${ item.id }` : item.id.toString();
        const d: ApiTreeSelectTreeData = {
            id: item.id,
            pId,
            key,
            value: key,
...
const ApiTreeSelect: React.FC<ApiTreeSelectProps> = (props) => {
